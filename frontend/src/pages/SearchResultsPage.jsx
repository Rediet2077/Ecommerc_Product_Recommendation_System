...
export default function SearchResultsPage() {
  ...
  const [filters, setFilters] = useState({ categoryId: null, minPrice: null, maxPrice: null, minRating: null })
  const limit = 12
  ...
  useEffect(() => {
    fetchResults()
  }, [q, page, sortBy, filters])

  const fetchResults = async () => {
    setLoading(true)
    try {
      const [sortField, sortOrder] = sortBy.split('-')
      const params = new URLSearchParams({ skip: (page-1)*limit, limit, sort_by: sortField, order: sortOrder })
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h1 className="text-lg font-bold text-gray-900">Search Results for "{q}"</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <select value={sortBy} onChange={(e) => { setSortBy(e.target.value); setPage(1) }} className="text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none bg-white">
          ...
          {loading ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {Array(8).fill(null).map((_, i) => <div key={i} className="bg-white rounded-xl border border-gray-100 overflow-hidden animate-pulse"><div className="aspect-square bg-gray-200"/><div className="p-3 space-y-2"><div className="h-3 bg-gray-200 rounded w-3/4"/><div className="h-3 bg-gray-200 rounded w-1/2"/></div></div>)}
            </div>
          ) : products.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-5xl mb-4">🔍</p>
              <h2 className="text-xl font-bold text-gray-900 mb-2">No results for "{q}"</h2>
